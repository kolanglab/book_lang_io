...
ここで本章の主役、**ファイルディスクリプタ（file descriptor、しばしば fd と略す）** を紹介します。これは、開いている I/O 対象を指し示す **小さな非負整数** にすぎません。`open` を呼ぶとカーネルがこの整数を一つ割り当てて返し、以降の `read`/`write`/`close` ではこの整数で対象を指定します。
