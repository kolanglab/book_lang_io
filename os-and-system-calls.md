...
> [!NOTE]
> もう一歩踏み込むと、fd という整数は、カーネル内の「開いているファイルの記録」（**open file description**）を指す番号札です。`dup` で fd を複製したり、`fork` で親子プロセスが fd を引き継いだりすると、**複数の fd が同じ記録を共有**します。このとき、読み書き位置（ファイルオフセット）も共有されるので、「同じファイルを指す 2 つの fd が独立した読み位置を持つ」とは限りません。また、子プロセスを起動する処理系では、fd が意図せず子へ引き継がれてリークしないよう、`exec` 時に自動で閉じる **close-on-exec**（`O_CLOEXEC`）という印を付けるのが現代の作法です。処理系の IO オブジェクトを設計するときは、「この fd は誰が閉じるのか・誰と共有されているのか」という所有権の意識が大切になります。
...
`errno` の中には、もう一つ処理系が必ず面倒を見ている常連がいます。**`EINTR`** です。ブロッキング中のシステムコールは、シグナル（プロセスへの非同期な通知）が届くと途中で中断され、`EINTR` という「失敗」で戻ることがあります。これはエラーというより「邪魔が入ったのでやり直して」という合図で、多くの場合は同じシステムコールを呼び直せば済みます。処理系の I/O 層は、内部でこのリトライを黙って行うことで、利用者に `EINTR` を見せないようにしています（タイムアウト付きの待ちなど、単純なやり直しでは済まない場合の扱いも、処理系が決めるべき仕様の一部です）。
