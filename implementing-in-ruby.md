...
> [!NOTE]
> 「複雑さを処理系に押し戻す」と言うとき、複雑さが**消えた**わけではない点に注意してください。ここには3つの層が登場します。
>
> 1. **アプリ利用者**（`Async do ... end` を書くだけの大多数）。複雑さはほぼ取り除かれます。
> 2. **スケジューラの実装者**（`async` gem の作者など、ごく少数）。`IO.select` 相当の多重化ロジックは、実はここへ移って集約されます。
> 3. **Ruby コア（MRI）本体**。フックとなる**インタフェース**（`Fiber#scheduler=` と一連のフックメソッド）だけを定義し、スケジューラの実装そのものは持ちません。
>
> つまり正確には「MRI 本体に複雑さが移った」のではなく、「全利用者が毎回書いていた複雑さが、再利用可能な**スケジューラ層の一点に集約**された」のです。複雑さの総量を減らすというより、**解決を一度きりにして使い回せる場所へ集める**——これが抽象化の本質であり、Fiber スケジューラの設計が優れている理由でもあります。
