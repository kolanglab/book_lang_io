...
第1章で「I/O は遅い」「待っている間 CPU は暇だ」という話をしました。本章では、その「待ち」の正体である **ブロッキング** を扱い、たくさんの I/O 相手を一人で同時にさばくための技術である **I/O 多重化（I/O multiplexing）** を、`select`、`poll`、`epoll` の順に学びます。ネットワークサーバや非同期処理を理解するための土台になります。
...
`gets` を呼んだとき、まだ入力がなければ、この行でプログラムは静止します。利用者がキーを打つまで、次の行は実行されません。このとき OS は、待っているプロセスを **スリープ状態** にして CPU を取り上げ、他の実行可能なプロセスに回します。一つのプログラムから見るかぎり、待っている間その CPU を無駄遣いしているわけではありません。
...
C10K を解くのは、**ノンブロッキング I/O** と **I/O 多重化** の組み合わせです。発想を転換します。一つずつ待つのではなく、たくさんの相手をまとめて OS に見張ってもらい、準備ができた相手だけを教えてもらうのです。
...
しかし `poll` も、本質的な問題（毎回すべての監視対象をカーネルへ渡し、カーネルが毎回全部を走査する点）は `select` と同じく抱えたままです。つまり O(N) の重さは残ります。少数の fd を時々見張るには十分ですが、数万の接続を高頻度でさばくには、まだ力不足でした。
...
肝は、毎回すべてを渡し直さないことです。登録はあらかじめ済ませてあり、`epoll_wait` は「いま準備ができているものだけ」を返します。このため、計算量は監視対象の総数 N ではなく、実際にイベントが起きた数だけに比例します。1 万接続のうち 3 つだけがアクティブなら、返ってくるのも 3 つだけです。これにより、接続数が増えても性能がほとんど落ちない、スケーラブルな多重化が実現しました。
...
`epoll` には通知の仕方が二種類あり、ここでつまずく人が多いので説明しておきます。
...
多重化が要るのはネットワークサーバだけではありません。子プロセスを起動して出力を受け取るだけのコードにも、ブロッキングの罠が潜んでいます。多重化の道具を手にしたいまなら、この罠を正しく理解して避けられます。

子プロセスの標準出力と標準エラー出力は、それぞれ別の **パイプ** で親とつながれます。パイプにはカーネル内のバッファがあり（Linux では既定 64 KiB 程度）、読み手が読まないまま満杯になると、書き手の `write` がブロックします。ここで次のコードを見てください。

```ruby
require "open3"
stdin, stdout, stderr, wait = Open3.popen3(
  "ruby", "-e", "STDERR.write('e' * 100_000); puts 'done'")
stdin.close
puts stdout.read    # 子の出力を最後まで読むつもり……が、ここで永遠に止まる
```

子は標準エラーへ 100,000 バイト書こうとしますが、親は誰も `stderr` を読んでいないので、パイプのバッファが満杯になった時点で子の `write` が止まります。一方の親は、`stdout.read` で「子が標準出力を閉じる（＝終了する）」のを待っています。子は親が stderr を読むのを待ち、親は子が終わるのを待つ。お互いに相手を待つ、典型的なデッドロックです。実際に動かすと、このプログラムは出力のないまま固まります。

```mermaid
graph LR
    P["親<br/>stdout.read で待機"] -.「子が終わるのを待つ」.-> C["子<br/>stderr への write で停止"]
    C -.「stderr が空くのを待つ」.-> P
```

原因が「一つの口だけをブロッキングで待ったこと」なら、解決はこの章で学んだとおりです。2 本のパイプを `IO.select` でまとめて見張り、読めるほうから読むのです。
...
bufs = { stdout => +"", stderr => +"" }   # io => 読み溜めたバイト列
until bufs.empty?
  readable, = IO.select(bufs.keys)        # どちらかが読めるまで待つ
  readable.each do |io|
    begin
      bufs[io] << io.readpartial(4096)    # 読めるぶんだけ読む
    rescue EOFError                       # この口は終わり
      io.close
      bufs.delete(io)
    end
  end
end
puts wait.value.exitstatus   # => 0（子は無事に終了できた）
```

stderr 側も読み進めるので、子の `write` は詰まらず、最後まで走って正常終了します。複数の I/O 相手をどれか一つでブロックせずにさばくという本章の主題が、サーバだけでなく、子プロセス 1 つ扱うだけのコードにも顔を出すわけです。

> [!TIP]
> 毎回これを書くのは大変なので、Ruby には `Open3.capture3` のように、内部でこの「両方読み」をやってくれる便利メソッドが用意されています（実装はスレッドで 2 本を並行に読む方式です）。`stdout` と `stderr` を両方受け取る場面では、生の `popen3` より、まずこれらの安全な高水準 API を使うのが定石です。ここでもまた「泥臭い部分を処理系・ライブラリの内側に隠す」という本書の主題が現れています。
...
- 子プロセスの stdout/stderr のような **2 本のパイプ** も、片方だけをブロッキングで待つと **パイプ詰まりのデッドロック** になる。多重化（または並行読み）で両方をさばく。
- 処理系がこれらの OS 差を吸収し、簡単な I/O インタフェースとして提供する。

`epoll` は「準備ができたか」を効率よく教えてくれますが、データのコピー自体はやはりアプリケーションが `read`/`write` で行います。次章では、その一歩先である「読み書きそのものをカーネルに任せる」非同期 I/O と `io_uring` へ進みます。
