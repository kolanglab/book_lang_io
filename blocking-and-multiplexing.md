...
## 身近な落とし穴：子プロセスのパイプ詰まり

多重化が要るのはネットワークサーバだけ、と思うかもしれません。実は、もっと身近な場面――**子プロセスを起動して出力を受け取る**だけのコード――にも、ブロッキングの罠が潜んでいます。多重化の道具を手にしたいまなら、この罠を正しく理解して避けられます。

子プロセスの標準出力と標準エラー出力は、それぞれ別の **パイプ** で親とつながれます。パイプにはカーネル内のバッファがあり（Linux では既定 64 KiB 程度）、**読み手が読まないまま満杯になると、書き手の `write` がブロック**します。ここで次のコードを見てください。
...
原因が「一つの口だけをブロッキングで待ったこと」なら、解決はこの章で学んだとおりです。**2 本のパイプを `IO.select` でまとめて見張り、読めるほうから読む**のです。
...
stderr 側も読み進めるので、子の `write` は詰まらず、最後まで走って正常終了します。「複数の I/O 相手を、どれか一つでブロックせずにさばく」という本章の主題が、サーバだけでなく、子プロセス 1 つ扱うだけのコードにも顔を出すわけです。
